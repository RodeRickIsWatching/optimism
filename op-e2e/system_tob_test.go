...
func TestMixedWithdrawalValidity(t *testing.T) {
	...
	for i := 0; i <= 8; i++ {
		...
		t.Run(fmt.Sprintf("withdrawal test#%d", i+1), func(t *testing.T) {
			...
			optimismPortal2, err := bindings.NewOptimismPortal2Caller(cfg.L1Deployments.OptimismPortalProxy, l1Client)
			require.NoError(t, err)
			...
			blockNumber, err := wait.ForOutputRootPublished(ctx, l1Client, cfg.L1Deployments.L2OutputOracleProxy, cfg.L1Deployments.OptimismPortalProxy, cfg.L1Deployments.DisputeGameFactoryProxy, receipt.BlockNumber)
			cancel()
			require.Nil(t, err)
