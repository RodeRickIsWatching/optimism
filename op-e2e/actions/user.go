...
type L1Bindings struct {
	...
	OptimismPortal2    *bindings.OptimismPortal2
...
func NewL1Bindings(t Testing, l1Cl *ethclient.Client) *L1Bindings {
	...
	optimismPortal2, err := bindings.NewOptimismPortal2(config.L1Deployments.OptimismPortalProxy, l1Cl)
	require.NoError(t, err)
...
func (s *CrossLayerUser) ProveWithdrawal(t Testing, l2TxHash common.Hash) common.Hash {
	...
	if e2eutils.UseFPAC() {
		...
		if err != nil {
			t.InvalidAction("no output has been published yet: %v", err)
			return common.Hash{}
		}
		...
		log.Warn("ree", "ree", l2OutputBlockNr, "ree2", l2OutputBlock.NumberU64(), "wd", l2WithdrawalBlock.NumberU64(), "name", t.Name())
		...
		if l2OutputBlock.NumberU64() < l2WithdrawalBlock.NumberU64() {
			t.InvalidAction("the latest L2 output is (%d, %d) and is not past L2 block %d that includes the withdrawal yet, no withdrawal can be proved yet", l2OutputBlock.NumberU64(), l2OutputBlockNr, l2WithdrawalBlock.NumberU64())
			return common.Hash{}
		}
	} else {
...
func (s *CrossLayerUser) ResolveClaim(t Testing, l2TxHash common.Hash) common.Hash {
	...
	portal2, err := bindings.NewOptimismPortal2(config.L1Deployments.OptimismPortalProxy, s.L1.env.EthCl)
	require.Nil(t, err)
	...
	resolveClaimTx, err := proxy.ResolveClaim(&s.L1.txOpts, common.Big0)
	require.Nil(t, err)
...
func (s *CrossLayerUser) Resolve(t Testing, l2TxHash common.Hash) common.Hash {
	...
	resolveTx, err := proxy.Resolve(&s.L1.txOpts)
	require.Nil(t, err)
