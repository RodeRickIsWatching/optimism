...
func ProveWithdrawal(t *testing.T, cfg SystemConfig, clients ClientProvider, l2NodeName string, ethPrivKey *ecdsa.PrivateKey, l2WithdrawalReceipt *types.Receipt) (withdrawals.ProvenWithdrawalParameters, *types.Receipt) {
	...
	blockNumber, err := wait.ForOutputRootPublished(ctx, l1Client, config.L1Deployments.L2OutputOracleProxy, config.L1Deployments.OptimismPortalProxy, config.L1Deployments.DisputeGameFactoryProxy, l2WithdrawalReceipt.BlockNumber)
	require.Nil(t, err)
	...
	portal2, err := bindings.NewOptimismPortal2Caller(config.L1Deployments.OptimismPortalProxy, l1Client)
	require.Nil(t, err)
...
func FinalizeWithdrawal(t *testing.T, cfg SystemConfig, l1Client *ethclient.Client, privKey *ecdsa.PrivateKey, withdrawalProofReceipt *types.Receipt, params withdrawals.ProvenWithdrawalParameters) (*types.Receipt, *types.Receipt, *types.Receipt) {
	...
	if e2eutils.UseFPAC() {
		portal2, err := bindings.NewOptimismPortal2(config.L1Deployments.OptimismPortalProxy, l1Client)
		require.Nil(t, err)
		...
		resolveClaimReceipt, err = geth.WaitForTransaction(resolveClaimTx.Hash(), l1Client, 3*time.Duration(cfg.DeployConfig.L1BlockTime)*time.Second)
		...
		resolveReceipt, err = geth.WaitForTransaction(resolveTx.Hash(), l1Client, 3*time.Duration(cfg.DeployConfig.L1BlockTime)*time.Second)
